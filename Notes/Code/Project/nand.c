...
int main (int argc, char **argv){
	...
	double Y[N],cost=ERROR+1; 					//Answer layer - holds what the expected output is
	...
	// inputs and desired outputs for NAND gate
...
//Training
	...
	//Done training, lets write to file
	FILE * bFile = fopen("B.neural","wb");
	fwrite(B, sizeof(double), N*(T+1), bFile);
	fclose(bFile);

	FILE * rFile = fopen("R.neural", "wb");
	fwrite(R, sizeof(double), N*N*(T+1), rFile);
	fclose(rFile);
